...
class CLI {
  constructor() {
    ...
    async getEmployees() {
        try {
            const sql = 'SELECT employees.id, employees.first_name, employees.last_name, roles.title, roles.department, roles.salary, employees.manager_id AS manager FROM employees JOIN roles ON employees.role_id = roles.id JOIN departments ON roles.department = departments.id ORDER BY employees.id';
            const res = await pool.query(sql);
            // console.log(res);
            // console.table(res.rows, ['id', 'first_name', 'last_name']);
