...
const questions = [
  ...
  'For which employee would you like to view the direct reports?'
];
...
class CLI {
  constructor() {}
    ...
    async getEmployeesByManager() {
      ...
      const sql = `SELECT id, CONCAT(first_name, ' ', last_name) AS name, manager_id  FROM employees WHERE manager_id IS NOT NULL;`
      pool.query(sql, (err, result) => {
        ...
        .then(res => {
          ...
          pool.query(sql, params, (err, result) => {
            ...
            if(result.rows.length === null || result.rows.length === 0) {
              console.log(`No employees found for the selected manager with id ${res.manager}`);
            } else {
              console.log('\n');
      ...
      pool.query(sqlDepartments, (err, result) => {
        ...
        .then(res => {
          ...
          pool.query(sql, [res.department], (err, result) => {
            ...
            if(result.rows.length === null || result.rows.length === 0) {
              console.log(`No Employees found for the ${res.department} department`);
            } else {
              console.log('\n');
              console.table(result.rows);
              console.log('\n');
