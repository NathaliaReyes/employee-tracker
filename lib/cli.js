      ...
      .then(res => {
        ...
        pool.query(sqlRole, (err, result) => {
          ...
          pool.query(sqlManager, (err, res) => {
            ...
            if(rol.length === 0 || manager.length === 0) {
              console.error('No roles or manager found!');
              return;
            }
            inquirer.prompt([
              {
    ...
    async updateEmployeeRole() {
      ...
      pool.query(sqlEmployee, (err, result) => {
        ...
        .then(choice => {
          ...
          const employeeList = choice.listEmployees;
          const rolList = choice.roleEmployees;

          params.push(employeeList);
          params.push(rolList);
          console.log(params);

          const sql = `UPDATE employees SET role_id = 3 WHERE first_name || ' ' || last_name = 'John Doe';`
          pool.query(sql, params, (err, result) => {
            if(err) throw err;
